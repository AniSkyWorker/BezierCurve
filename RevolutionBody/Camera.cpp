...
namespace
{
	...
	const float PIXEL_INFELICITY = 0.01f;
...
void CCamera::OnDragMotion(const glm::vec2 &pos)
{
	if (m_isRotating)
	{
		...
		m_dragBeginPos = pos;
		m_rotationRadians.x += offset.x * PIXEL_INFELICITY;
		m_rotationRadians.y += offset.y * PIXEL_INFELICITY;
	}
}
...
glm::mat4 CCamera::GetViewTransform()
{
	...
	direction = glm::rotateY(glm::normalize(direction), m_rotationRadians.x);
	direction = glm::rotateX(glm::normalize(direction), m_rotationRadians.y);
